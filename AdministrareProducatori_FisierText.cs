...
namespace NivelStocareDate
{
    public class AdministrareProducatori_FisierText
    {
        ...
        public Producator[] CitesteFisier(string nume_Fisier)
        {
            Producator[] producatori = new Producator[10];
            using (StreamReader streamReader = new StreamReader(nume_Fisier))
            {
                string linieFisier;
                while ((linieFisier = streamReader.ReadLine())!=null)
                {
                    string[] campuri = linieFisier.Split(',');
                    string nume = campuri[0];
                    Producator producator = new Producator(nume);
                   //producatori.AddProducator(producator);
